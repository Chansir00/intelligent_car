...
uint8_t L_search_amount = 140, R_search_amount = 140;  //左右边界搜点时最多允许的点
uint16_t L_corner_flag = 0;//左拐点存在标志
const uint16_t Boundary_search_end = 10; // Define Boundary_search_end with an appropriate value
uint16_t L_corner_row = 0;//左拐点所在行
uint16_t L_corner_col = 0;//左拐点所在列
uint16_t L_corner_angle = 0;//左拐点角度
uint16_t R_corner_flag = 0;//右拐点存在标志
uint16_t R_corner_row = 0;//右拐点所在行
uint16_t R_corner_col = 0;//右拐点所在列
uint16_t R_corner_angle = 0;//右拐点角度
uint16_t enable_L_corner=1,enable_R_corner=1;
uint8_t left_findflag = 0; // 左边找到标志
uint8_t right_findflag = 0; // 右边找到标志
uint16_t left_lose = 0; // 初始化左边丢失标志
uint16_t right_lose = 0; // 初始化右边丢失标志
uint16_t L_start_x = 0; // 初始化左边界起始点x坐标
uint16_t L_start_y = 0; // 初始化左边界起始点y坐标
uint16_t R_start_x = 0; // 初始化右边界起始点x坐标
uint16_t R_start_y = 0; // 初始化右边界起始点y坐标
...
void search_neighborhood(void)
{
    ...
    if(left_findflag)//如果左边界点存在并找到,则开始爬线
    {
        ...
        L_edge[0].flag = 1;
        uint16_t curr_row = L_start_y;//初始化行坐标
        uint16_t curr_col = L_start_x;//初始化列坐标
        ...
        for(int i = 1;i < L_search_amount; i++)    //最多搜索150个点
        {
            ...
            //搜线过程
            if(dire_left != 2&&Image_use_out[curr_row-1][curr_col-1]==0 &&Image_use_out[curr_row-1][curr_col]==1)   //左上黑，2，右边白
            {
                curr_row = curr_row -1;
                curr_col = curr_col -1;
            ...
            else if(dire_left!=3&&Image_use_out[curr_row-1][curr_col+1]==0&&Image_use_out[curr_row][curr_col+1]==1)    //右上黑，3，下边白
            {
                curr_row = curr_row -1;
                curr_col = curr_col + 1;
                L_edge_count = L_edge_count + 1;
            ...
            else if(Image_use_out[curr_row-1][curr_col]==0&&Image_use_out[curr_row-1][curr_col+1]==1)                  //正上黑，1，右白
            {
                curr_row = curr_row - 1;
                L_edge_count = L_edge_count + 1;
                dire_left = 0;
            ...
            else if(dire_left!=5&&Image_use_out[curr_row][curr_col-1]==0&&Image_use_out[curr_row-1][curr_col-1]==1)     //正左黑，5，上白
            {
                curr_col = curr_col - 1;
                L_edge_count = L_edge_count +1;
                dire_left = 4;
            ...
            else if(dire_left!=4&&Image_use_out[curr_row][curr_col+1]==0&&Image_use_out[curr_row+1][curr_col+1]==1)  //正右黑，4，下白
            {
                curr_col = curr_col + 1;
                L_edge_count = L_edge_count +1;
                dire_left = 5;
            ...
            else if(dire_left!=6&&Image_use_out[curr_row+1][curr_col-1]==0&&Image_use_out[curr_row][curr_col-1]==1)    //左下黑，6，上白
            {
                curr_row = curr_row + 1;
                curr_col = curr_col -1;
                L_edge_count = L_edge_count +1;
            ...
            else if(dire_left!=7&&Image_use_out[curr_row+1][curr_col+1]==0&&Image_use_out[curr_row+1][curr_col]==1)    //右下黑，7，左白
            {
                curr_row = curr_row + 1;
                curr_col = curr_col + 1;
                L_edge_count = L_edge_count +1;
    ...
    if(right_findflag)//如果右边界存在并搜到
		...
		////越界退出 行越界和列越界（向上向下向左向右）
                     ...
                     else if(dire_right!=2&&Image_use_zip[curr_row-1][curr_col-1]==0&&Image_use_zip[curr_row][curr_col-1]==1) //左上黑，2，下白
                     {
                         curr_row = curr_row-1;
                         curr_col = curr_col-1;
                         R_edge_count = R_edge_count + 1;
                     ...
                     else if(Image_use_zip[curr_row-1][curr_col]==0&&Image_use_zip[curr_row-1][curr_col-1]==1)                  //正上黑，1，左白
                     {
                         curr_row = curr_row - 1;
                         R_edge_count = R_edge_count + 1;
                         dire_right = 0;
                     ...
                     else if(dire_right!=4&&Image_use_zip[curr_row][curr_col+1]==0&&Image_use_zip[curr_row-1][curr_col+1]==1)   //正右黑，4，上白
                     {
                         curr_col = curr_col + 1;
                         R_edge_count = R_edge_count + 1;
                         dire_right = 5;
                     ...
                     else if(dire_right!=5&&Image_use_zip[curr_row][curr_col-1]==0&&Image_use_zip[curr_row+1][curr_col-1]==1)   //正左黑，5，下白
                     {
                         curr_col = curr_col-1;
                         R_edge_count = R_edge_count + 1;
                         dire_right = 4;
                     ...
                     else if(dire_right!=6&&Image_use_zip[curr_row+1][curr_col-1]==0&&Image_use_zip[curr_row+1][curr_col]==1)   //左下黑，6，右白
                     {
                         curr_row = curr_row + 1;
                         curr_col = curr_col - 1;
                         R_edge_count = R_edge_count + 1;
                     ...
                     else if(dire_right!=7&&Image_use_zip[curr_row+1][curr_col+1]==0&&Image_use_zip[curr_row][curr_col+1]==1)   //右下黑，7，上白
                     {
                         curr_row = curr_row + 1;
                         curr_col = curr_col + 1;
                         R_edge_count = R_edge_count + 1;
